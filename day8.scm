...
(define-public (solve2 input)
               (letrec ((junction-boxes (parse-coords input))
                        (junction-pairs (sorted-by-distance (to-junction-pairs junction-boxes)))
                        (circuits (to-circuit-set junction-boxes))
                        (num-circuits (length junction-boxes))
                        (last-joined #f))
                 (while (> num-circuits 1)
                        (let ((next (first junction-pairs)))
                          (when (connect circuits next)
                            (set! num-circuits (1- num-circuits))
                            (set! last-joined next))
                          (set! junction-pairs (cdr junction-pairs))))
                 (* (coord-x (car last-joined)) (coord-x (cdr last-joined)))))
